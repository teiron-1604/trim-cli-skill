...
```bash
trim-cli app list
trim-cli app status trim.alist
trim-cli app store list
trim-cli app store search alist
trim-cli app store detail trim.alist --source-id 265 --version 3.0.13
trim-cli app store latest-release
trim-cli app check-update
trim-cli app check-update --post --yes
trim-cli app common volume
trim-cli app common volume --set 2 --yes
trim-cli app config sys
trim-cli app config detail trim.alist
trim-cli app service list
trim-cli app entry list --only-hidden
trim-cli app desktop list
trim-cli app config set-sys --fields '{"autoRunNewApp":true}' --yes
trim-cli app config set trim.alist --fields '{"services":[],"extraAuthorizationPath":[]}' --yes
trim-cli app config auto-update trim.alist --enable --yes
trim-cli app config proxy-access trim.alist --allowed --yes
trim-cli app config post-wizard trim.alist --fields '{"customParameters":[]}' --yes
trim-cli app guide check-installed trim.media trim.photos
trim-cli app guide batch-download-install 2 trim.media trim.photos --yes
trim-cli app operation settings
trim-cli app openapi app-detail trim.alist
trim-cli app openapi allow-user-auth-paths trim.alist
trim-cli app openapi add-user-auth-path trim.alist /vol1/1000/docs --yes
trim-cli app openapi add-user-auth-inherit-file trim.alist /vol1/1000/docs/file.txt --yes
trim-cli app sac app-store-list
trim-cli app task-status <taskId>
trim-cli app task-cancel <taskId> --yes
trim-cli app download cancel <downloadTaskId> --yes
trim-cli app install-cancel <taskId> --yes
trim-cli app update-cancel <taskId> --yes
trim-cli app install trim.alist --version 3.0.13 --source-id 265 --volume-id 2 --dry-run --yes
trim-cli app install trim.alist --version 3.0.13 --source-id 265 --volume-id 2 --yes
trim-cli app install trim.alist --version 3.0.13 --volume-id 2 --custom-parameters '[{"key":"port","value":8080}]' --api-scope '{"API.User.FileAccess":true}' --yes
trim-cli app install trim.alist --version 3.0.13 --volume-id 2 --accept-license --cancel-on-failure --yes
trim-cli app install-fpk ~/Downloads/demo.fpk --volume-id 2 --dry-run --yes
trim-cli app install-fpk ~/Downloads/demo.fpk --volume-id 2 --yes
trim-cli app update trim.alist --version 3.0.14 --volume-id 2 --dry-run --yes
trim-cli app update trim.alist --version 3.0.14 --volume-id 2 --yes
trim-cli app stop trim.alist --yes
trim-cli app start trim.alist --yes
trim-cli app uninstall trim.alist --yes
```
