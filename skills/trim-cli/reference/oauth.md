...
- `scope`: `trim.user.all trim.file.read trim.file.write trim.download.all trim.appcenter.all trim.photo.all trim.baidu_netdisk.all trim.system.all trim.log.all trim.storage.all trim.docker.all`
...
`trim.basic` 不属于当前可申请列表，CLI 不显式申请。旧的 `file photo` scope 或缺少
`trim.baidu_netdisk.all` 的 session 不再兼容，必须重新登录；refresh token 不能用来扩展
原授权范围。
